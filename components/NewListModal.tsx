...
import Readlist, { createReadlist } from "@/utils/supabase/api/readlists";
import { createClient } from "@/utils/supabase/client";

export default function NewListModal({ onListCreated, onCancel } : { onListCreated: (e: Readlist) => void, onCancel: () => void}) {

  const [name, setName] = useState<string>('')

  const createList = async () => {
    const readList = await createReadlist(createClient(), name)
    onListCreated(readList)
  }
    ...
    <>
      <div
        ...
        <div onClick={(e) => e.stopPropagation()} className="bg-background rounded-md p-4 z-10">
          <h1 className="text-2xl font-bold ml-2 mb-6">New List</h1>
